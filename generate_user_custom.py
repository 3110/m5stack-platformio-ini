...
with open(os.path.join(firmware_dir, firmware_version_file), "r") as f:
    for line in f:
        ...
        raise RuntimeError("Version is not found in %s" % firmware_version_file)
...
firmware_bin = "%s_%s_firmware_%s.%s" % (
    firmware_target,
    firmware_name,
    firmware_version,
    firmware_suffix,
